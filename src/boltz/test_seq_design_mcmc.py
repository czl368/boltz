...
# Default Parameters
DATA_PATH = Path("./6d40_short_prot_random_seq.yaml")
...
def main():
    ...
    for iteration in range(MAX_ITERATIONS):
        ...
        cur_pred_dir = OUT_DIR / "predictions" / f"iteration_{iteration+1}"
        ...
        new_plddt = plddt_score

        acceptance_prob = acceptance_probability(best_plddt, new_plddt, temperature)
        if np.random.rand() < acceptance_prob:
            best_plddt = new_plddt
            best_sequence = new_sequence
            print(f"Accepted new sequence with pLDDT {new_plddt:.2f} (T={temperature:.2f})")
        else:
            print(f"Rejected new sequence (pLDDT {new_plddt:.2f}), keeping current best.")
